...
class Reconstruction_autoencoder(nn.Module):
    def __init__(self, enc_dim):
        ...
        self.fc = nn.Linear(enc_dim, 4 * 1615)
    ...
    def forward(self, z):
        z = self.fc(z).view((z.shape[0], 4, 1615))
        z = self.activation(self.bn1(z))
        z = self.layer1(z)
        z = self.layer2(z)
        z = nn.functional.interpolate(z, scale_factor=8, mode="linear", align_corners=True)
        z = self.layer3(z)
        z = nn.functional.interpolate(z, scale_factor=5, mode="linear", align_corners=True)
        z = self.layer4(z)
        return z
...
class Conversion_autoencoder(nn.Module):
    def __init__(self, num_nodes, enc_dim, nclasses=2):
        ...
        self.conv1 = nn.Conv1d(1, 16, kernel_size=15, stride=2, padding=1, bias=False)
        ...
        self.layer1_i = nn.Sequential(
            nn.ConvTranspose1d(256,256,3,2,1, output_padding=1),
        ...
        self.layer3_i = nn.Sequential(
            nn.ConvTranspose1d(64, 64, 3, 2, 1, output_padding=1),
            PreActBlock(64, 32, 1),
            PreActBlock(32, 16, 1)
        )
        self.layer4_i = nn.Sequential(
            nn.ConvTranspose1d(32, 32, 15, 2, 1, output_padding=1),
            PreActBlock(32, 8, 1),
            PreActBlock(8, 4, 1),
            PreActBlock(4, 1, 1),
        )
...
class MultiTaskRawNet(nn.Module):
    def __init__(self,
